...
class PackageValidationTest(unittest.TestCase):
    ...
    def test_guided_review_is_manual_and_preserves_human_control(self) -> None:
        ...
        self.assertIn("one visible review card", body)
        self.assertIn("exact, focused code, diff, or artifact excerpt", body)
        self.assertIn("binary or otherwise non-renderable artifacts", body)
        self.assertIn("proposed fixes remain drafts", body)
