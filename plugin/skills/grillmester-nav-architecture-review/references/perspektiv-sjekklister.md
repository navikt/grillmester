...
Sjekklistene dekker Nav-spesifikke temaer som kan være relevante. De er
spørsmålsbank, ikke policy eller fasit. Verifiser plattformnavn, støttede
mekanismer og anbefalinger mot oppdatert, autoritativ dokumentasjon, og bruk
bare punktene som gjelder consumerens faktiske kontekst.
...
Fokus: Nav-spesifikk auth, `accessPolicy`, PII og personvern. Dypere trusselmodellering → `grillmester-security-review`.
...
- **Drift og beredskap:**
  - Runbook for vanlige feilmoduser (se også `grillmester-nav-troubleshoot`).
