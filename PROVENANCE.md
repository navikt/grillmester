...
[`policy/client-artifacts.json`](policy/client-artifacts.json) records the exact
upstream artifacts used by compatibility gates and the optional high-assurance
manager. It is verification metadata, not evidence that the standard
Grillmester distribution owns or bundles those client executables. Only a user
who explicitly selects the manager's contract gets its checksum validation and
private per-session `trusted-bin` staging.

## Derived terminal artifacts

`targets/opencode-v1/` is a deterministic client projection of the canonical
plugin. The two local-model targets, `targets/opencode-v1-focused/` and
`targets/copilot-cli-focused-v1/`, are deterministic derivatives of that full
OpenCode target and the canonical Copilot plugin under
`policy/focused-context-v1.json`. They contain no separately imported agent or
skill source. Their manifests bind the focused policy, the relevant full-source
manifest, every generated byte and every file mode. Release construction reruns
the independent projection check before accepting their inner manifests, so a
self-consistent re-manifest of modified focused content is not provenance.

`scripts/grillmester.py`, `scripts/grillmester_local.py` and the deterministic
local smoke harness are repository-owned runtime infrastructure. They bind the
reviewed payload to user-installed clients and a user-owned model server; they
do not add prompt/content lineage or transfer ownership of those external
