...
archive locations, sizes, rosters and digests used by release CI. It downloads
and verifies release-test inputs, but is not a runtime pin, a shipped client or
evidence that Grillmester owns or privately stages those executables.
...
plugin. `plugin/manifest.json` is a deterministic inventory of every other
canonical Copilot payload byte and file mode; it is generated locally and
contains no imported content of its own. The two local-model targets,
`targets/opencode-v1-focused/` and
...
the independent full-manifest and projection checks before accepting their
inner manifests, so a self-consistent re-manifest of modified content is not
provenance.
