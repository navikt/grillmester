...
| Bounded Grillmester slice: "Legg til valideringen av organisasjonsnummer som er beskrevet i issue #123." The change is R1 and touches one module and its test. | State in one sentence that Grillmester implements the slice itself and why. Implement it without a Kokk brief, run the slice's verification, and report fresh evidence. Do not run a separate `/review` self-pass over its own slice. |
| Risky Grillmester slice: an R3 change to an authorization check across several services. | State in one sentence that the slice goes to Kokk because it is R3. Delegate with a Kokk brief and require Grill-inspektør through the portable R3/R4 route. If the user asks Grillmester to implement it itself, note once that Inspector then reviews code from the same model family, and follow the user's choice. |
...
| Loose design concept: "Vi starter et løst definert eksperiment og vil lage konseptskisser som skal ende i Figma." | Designer loads `grilling` before the first sketch and asks one question at a time, with a recommendation, about goal, audience, assumptions, and what the exploration should teach. It moves to sketches once the direction gives genuinely different alternatives, and does not route to an implementation agent. |
