...
| Bounded Grillmester slice: "Legg til valideringen av organisasjonsnummer som er beskrevet i issue #123." The change is R1 and touches one module and its test. | State in one sentence that Grillmester implements the slice itself and why. Record the slice boundary, implement it without a Kokk brief, run the slice's verification and the `/review` self-pass, and report fresh evidence. |
| Risky Grillmester slice: an R3 change to an authorization check across several services. | State in one sentence that the slice goes to Kokk because it is R3. Delegate with a Kokk brief and follow the portable R3/R4 review route: Inspector approval, accepted concerns, or a human waiver. If the user asks Grillmester to implement it itself, note once that this removes the separate implementer, and follow the user's choice. |
